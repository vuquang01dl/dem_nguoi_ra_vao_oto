...
def update_frame():
    ...
    # Vẽ hình chữ nhật
    if rect_in_defined:
        cv2.rectangle(frame_copy, start_point_in, end_point_in, (0, 0, 255), 2)  # vùng vào: đỏ
    if rect_out_defined:
        cv2.rectangle(frame_copy, start_point_out, end_point_out, (0, 255, 0), 2)  # vùng ra: xanh lá
...
select_in_button = Button(window, text="Chọn ô vào", font=("Arial", 14), command=select_in_area)
select_in_button.pack(pady=5)

select_out_button = Button(window, text="Chọn ô ra", font=("Arial", 14), command=select_out_area)
